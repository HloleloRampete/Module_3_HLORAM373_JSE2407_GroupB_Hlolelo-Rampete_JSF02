  ...
  function addToFavourites(event) {
    ...
    // Replace the following with your actual logic to add to favourites
    console.log(`Product ${id} added to favourites`);
    // You might want to call a store action or an API here
  }
</script>
  ...
  <div class="flex-1 flex flex-col p-6">
    <header class="mb-2 flex-2">
      <h2 class="text-lg line-clamp-2 font-extrabold leading-snug">
        <a href={`/products/${id}`} class="text-slate-600 focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300">
          {title}
        </a>
      </h2>
    </header>
